...
def binary_cross_entropy(weights_tensor, nn, input,
                         correct_output, reg, reg_params):
    output = nn.output(input, weights_tensor)
    epsilon = 1e-15
...
def hinge_loss(weights_tensor, nn, input, correct_output, reg, reg_params):
    output = nn.output(input, weights_tensor)
    return (np.mean(np.maximum(0, 1 - output * correct_output)) +
            reg(weights_tensor, *reg_params))
