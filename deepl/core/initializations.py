...
def zero_init(structure):
    return  [np.zeros((structure[i+1], structure[i]+1)) 
...
def uniform_init(structure, a, b):
    return  [np.random.uniform(low=a, high=b, size=(structure[i+1], structure[i]+1))
             for i in range(len(structure)-1)]
...
def xavier_init(structure):
    ...
    return [np.random.uniform(low=-limit, high=limit,
            size=(structure[i+1], structure[i]+1)) for i in range(len(structure)-1)]
...
def constant_init(structure, c):
    return [np.full((structure[i+1], structure[i]+1), c) 
            for i in range(len(structure)-1)]
